...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 14 15:24:03 2017

@author: zhangchi
"""

# enron Dataset Preprocess
...
def readTestNet(oldG, input_file, now, sep_char=' '): 
    num_lines = 0
    G = nx.Graph()
    for line in open(input_file):
        num_lines += 1
        ...
        if line[1] == line[2]:
            ...
            if oldG.has_node(line[1]) and oldG.has_node(line[2]):
                if oldG.has_edge(line[1], line[2]) is False:            
...
def getWeight(timeList, now):
    ...
    for time in timeList:
        if time <= now:
            ...
            possibility = possibility * (1 - np.exp((time-now)/1209600.))
...
def updateWeight(G, now):
    for nodeA, nodeB in G.edges():
        ...
        G[nodeA][nodeB]['prob'] = getWeight(timeList,now)
        G[nodeA][nodeB]['time'] = []
    return G
...
# =============================================================================
# newG = readNet('resultFullData.txt', 927590400+1209600*54)
# newG = updateWeight(newG, 927590400+1209600*54)
# G = readTestNet(newG, 'resultFullData.txt', 927590400+1209600*54)
# =============================================================================
...
for para in paraList:
    for mode in modeList:
        ...
        testNumber = 8
        for testCount in xrange(1,testNumber):
            newG = readNet('resultFullData.txt', 927590400+1209600*testCount*10)
            newG = updateWeight(newG, 927590400+1209600*testCount*10)
            G = readTestNet(newG, 'resultFullData.txt', 927590400+1209600*testCount*10)
# =============================================================================
